...
# --- Simulate the step input ---
def step_reference(t):
    """Unit step reference: 0m for t<0, 1m for t>=0"""
    return 1.0  # Cart should move to x=1m

def lqr_tracking_system(t, X, A, B, K):
    """LQR controller tracking a step reference"""
    x, x_dot, theta, theta_dot = X
    
    # Reference state: cart at x_ref=1m, pendulum upright
    x_ref = step_reference(t)
    X_ref = np.array([x_ref, 0.0, 0.0, 0.0])
...
# Solve
sol = solve_ivp(lqr_tracking_system, [0, t_end], X0, t_eval=t_eval,
...
ani.save('controlled_unit_step.mp4', writer='ffmpeg', fps=fps)
...
print(f"{'='*60}")
# print(f"Initial angle: {theta0 * 180/np.pi:.2f}°")
...
for angle_deg in test_angles:
    ...
    sol_test = solve_ivp(lqr_tracking_system, [0, t_end], X0_test,
